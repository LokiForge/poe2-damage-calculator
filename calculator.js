...
export function marginal(s){const current=calculate(s).dps;const items=[['base','技能基础伤害','+1 点'],['added','附加基础伤害','+1 点'],['inc','Increased 增伤','+1 个百分点'],['rate','每秒命中次数','+1 次/秒'],['crit','最终暴击率','+1 个百分点'],['bonus','最终暴击伤害加成','+1 个百分点']].map(([key,name,unit])=>({name,unit,change:d=>{d[key]=key==='crit'?Math.min(100,d[key]+1):d[key]+1;}}));s.more.forEach((m,i)=>items.push({name:m.name||`独立倍率 ${i+1}`,unit:'+1 个百分点（已有 More）',change:d=>d.more[i].value++}));items.push({name:'新增独立 More',unit:'新增 ×1.01 倍率',change:d=>d.more.push({name:'新增',value:1})});return items.map(({name,unit,change})=>{const next=structuredClone(s);change(next);const delta=calculate(next).dps-current;return {name,unit,delta,percent:current===0?null:delta/current*100};});}
